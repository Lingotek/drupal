 ...
 *   },
 *   batch = {
 *     "title" = @Translation("Uploading content to Lingotek service")
 *   }
 * )
 */
class Upload extends LingotekFormComponentBulkActionBase {
  ...
  public function executeSingle(ContentEntityInterface $entity, $options, LingotekFormComponentBulkActionExecutor $executor, array &$context) {
    ...
    if (!$entity->getEntityType()->isTranslatable() || !$bundleInfos[$entity->bundle()]['translatable']) {
      $this->messenger()->addWarning($this->t('Cannot upload @type %label. That @bundle_label is not enabled for translation.',
    ...
    if (!$this->lingotekConfiguration->isEnabled($entity->getEntityTypeId(), $entity->bundle())) {
      $this->messenger()->addWarning($this->t('Cannot upload @type %label. That @bundle_label is not enabled for Lingotek translation.',
        ['@type' => $bundleInfos[$entity->bundle()]['label'], '%label' => $entity->label(), '@bundle_label' => $entity->getEntityType()->getBundleLabel()]
      ));
      return FALSE;
    }
    if ($profile = $this->lingotekConfiguration->getEntityProfile($entity)) {
      try {
        $this->translationService->uploadDocument($entity, $options['job_id']);
      }
        ...
        // Retry now that we have the proper document id.
        return $this->executeSingle($entity, $options, $executor, $context);
      }
        ...
        if ($this->translationService->getDocumentId($entity)) {
          $this->messenger()->addError($this->t('The update for @entity_type %title failed. Please try again.', ['@entity_type' => $entity->getEntityTypeId(), '%title' => $entity->label()]));
        }
        else {
          $this->messenger()->addError($this->t('The upload for @entity_type %title failed. Please try again.', ['@entity_type' => $entity->getEntityTypeId(), '%title' => $entity->label()]));
        }
        return FALSE;
      }
    }
