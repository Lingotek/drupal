...
/**
 * Defines a Lingotek bulk action plugin for the check translation operation for
 * a single language.
 *
 * @LingotekFormComponentBulkAction(
 *   id = "check_translation",
 *   title = @Translation("Check progress of %language (%langcode) translation"),
 *   group = @Translation("Check translation progress"),
 *   weight = 60,
 *   form_ids = {
 *     "lingotek_management",
 ...
 *   },
 *   batch = {
 *     "title" = @Translation("Checking translations status from the Lingotek service.")
 *   },
 *   deriver = "Drupal\lingotek\Plugin\LingotekFormComponent\BulkAction\Derivative\LanguageLingotekBulkActionDeriver",
 * )
 */
class CheckTranslation extends LingotekFormComponentBulkActionBase {
  ...
  public function executeSingle(ContentEntityInterface $entity, $options, LingotekFormComponentBulkActionExecutor $executor, array &$context) {
    $language = $this->pluginDefinition['langcode'];
    ...
    if (!$entity->getEntityType()->isTranslatable() || !$bundleInfos[$entity->bundle()]['translatable']) {
      $this->messenger()->addWarning($this->t('Cannot check @type %label translation to @language. That @bundle_label is not enabled for translation.',
    ...
    if (!$this->lingotekConfiguration->isEnabled($entity->getEntityTypeId(), $entity->bundle())) {
      $this->messenger()->addWarning($this->t('Cannot check @type %label translation to @language. That @bundle_label is not enabled for Lingotek translation.',
        ['@type' => $bundleInfos[$entity->bundle()]['label'], '%label' => $entity->label(), '@bundle_label' => $entity->getEntityType()->getBundleLabel(), '@language' => $language]
      ));
      return FALSE;
    }
    if ($profile = $this->lingotekConfiguration->getEntityProfile($entity, FALSE)) {
      try {
        ...
        $this->messenger()->addError($this->t('The request for @entity_type %title translation status failed. Please try again.', ['@entity_type' => $entity->getEntityTypeId(), '%title' => $entity->label()]));
        return FALSE;
      }
    }
    else {
