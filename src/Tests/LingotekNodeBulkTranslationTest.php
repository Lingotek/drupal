...
class LingotekNodeBulkTranslationTest extends LingotekTestBase {
  ...
  /**
   * Tests that a node cannot be downloaded when source was edited using the
   * links on the management page.
   */
  public function testNodeTranslationIsNotDownloadedIfEditedUsingLinks() {
    // Login as admin.
    $this->drupalLogin($this->rootUser);
    ...
    // We edit our node, so the translations need to be checked again.
    $edit = array();
    ...
    // Try to download the Spanish translation.
    $this->assertLinkByHref($basepath . '/admin/lingotek/entity/download/dummy-document-hash-id/es_MX?destination=' . $basepath .'/admin/lingotek/manage/node');
    $this->clickLink('ES');
    $this->assertText('The translation of node Llamas are cool EDITED into es_MX failed because it has been edited since being uploaded.');
  }
  ...
  /**
   * Tests that a node cannot be downloaded when source was edited using the
   * actions on the management page.
   */
  public function testNodeTranslationIsNotDownloadedIfEditedUsingActions() {
    // Login as admin.
    $this->drupalLogin($this->rootUser);
    ...
    // Check status of the German translation.
    $this->assertLinkByHref($basepath . '/admin/lingotek/entity/check_target/dummy-document-hash-id/de_AT?destination=' . $basepath .'/admin/lingotek/manage/node');
    $edit = [
      'table[1]' => TRUE,  // Node 1.
      'operation' => 'check_translation:de'
    ...
    // Try to download the German translation.
    $this->assertLinkByHref($basepath . '/admin/lingotek/entity/download/dummy-document-hash-id/de_AT?destination=' . $basepath .'/admin/lingotek/manage/node');
    $edit = [
      'table[1]' => TRUE,  // Node 1.
      'operation' => 'download:de'
    ...
    $this->drupalPostForm(NULL, $edit, t('Execute'));
    $this->assertText('The translation of node Llamas are cool EDITED into de_AT failed because it has been edited since being uploaded.');
  }
