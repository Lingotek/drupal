...
class LingotekToolbarIntegrationTest extends LingotekTestBase {
  ...
  public function testToolbarItems() {
    ...
    $response = $this->drupalGetAjax('/toolbar/subtrees/' . $settings['toolbar']['subtreesHash']);
    $this->assertResponse('200');
    $this->assertEqual($response[0]['command'], 'setToolbarSubtrees', 'Subtrees response uses the correct command.');
    $this->assertEqual(array_key_exists('lingotek-config-dashboard', $response[0]['subtrees']), 'There is a subtree for Lingotek config.');
    ...
    $this->assertRaw('<a href="' . $basepath . '/admin/lingotek/manage" title="Manage content translations using Lingotek cloud-based localization" id="toolbar-link-lingotek-manage" class="toolbar-icon toolbar-icon-lingotek-manage" data-drupal-link-system-path="admin/lingotek/manage">Content</a>',
      'There is an expanded item for the Content bulk management.');
    $this->assertRaw('<a href="' . $basepath . '/admin/lingotek/manage/node" id="toolbar-link-lingotek-manage-node" class="toolbar-icon toolbar-icon-lingotek-manage-content:lingotek-manage-node" title="" data-drupal-link-system-path="admin/lingotek/manage/node">Content</a>',
      'There is an item for the Nodes in Content bulk management.');
    $this->assertRaw('<a href="' . $basepath . '/admin/lingotek/config/manage" title="Manage config translations using Lingotek cloud-based localization" id="toolbar-link-lingotek-manage_config" class="toolbar-icon toolbar-icon-lingotek-manage-config" data-drupal-link-system-path="admin/lingotek/config/manage">Config</a>',
      'There is an item for Config bulk management.');
    $this->assertRaw('<a href="' . $basepath . '/admin/lingotek/settings" title="Lingotek configuration" id="toolbar-link-lingotek-settings" class="toolbar-icon toolbar-icon-lingotek-settings" data-drupal-link-system-path="admin/lingotek/settings">Settings</a>',
      'There is an item for Lingotek Settings.');

    $html = $response[0]['subtrees']['system-admin_config'];
    $this->setRawContent($html);
    $this->verbose($this->getRawContent());
    $this->assertRaw('<a href="' . $basepath . '/admin/lingotek" title="Convenient cloud-based localization and translation by Lingotek" id="toolbar-link-lingotek-dashboard" class="toolbar-icon toolbar-icon-lingotek-config-dashboard" data-drupal-link-system-path="admin/lingotek">Lingotek Translation</a>',
