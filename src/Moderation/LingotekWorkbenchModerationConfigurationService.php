...
class LingotekWorkbenchModerationConfigurationService implements LingotekModerationConfigurationServiceInterface {
  ...
  public function getUploadStatus($entity_type_id, $bundle) {
    ...
    $upload_status = $config->get('translate.entity.' . $entity_type_id . '.' . $bundle . '.workbench_moderation.upload_status');
  ...
  public function getDownloadTransition($entity_type_id, $bundle) {
    $config = $this->configFactory->get('lingotek.settings');
    $download_transition = $config->get('translate.entity.' . $entity_type_id . '.' . $bundle . '.workbench_moderation.download_transition');
  ...
  public function setUploadStatus($entity_type_id, $bundle, $status) {
    ...
    $config->set('translate.entity.' . $entity_type_id . '.' . $bundle . '.workbench_moderation.upload_status', $status);
    $config->save();
    return $status;
  }
  ...
  public function setDownloadTransition($entity_type_id, $bundle, $transition) {
    $config = $this->configFactory->getEditable('lingotek.settings');
    $config->set('translate.entity.' . $entity_type_id . '.' . $bundle . '.workbench_moderation.download_transition', $transition);
    $config->save();
    return $transition;
  }
